...
#Objetivo: Realizar un informe donde se pueda evidenciar el armado de un proyecto a partir de las siguientes etapas:
...
#1. 
#El objetivo de este trabajo es analizar las ventas de las propiedades en CABA en el año 2020.
#Para ello nos enfocaremos en responder la siguiente pregunta: ¿Cuál es la relación entre el transporte público, más precisamente el subte, y la venta de propiedades en CABA? 
#En este sentido, procederemos a descargar la base de datos de Properati para observar la venta de inmuebles y los datos del Cronista para obtener el valor del dólar a la fecha de estudio.
...
# Para poder analizar el valor actual de las propiedades en USD, vamos a trabajar con la cotización del dia.
# Para esto tomaremos los datos de la página web del cronista y haremos un Scrapping

# Primero cargamos el URL de la página web
...
# Veremos la información volcada en un mapa  de la ciudad con la red de subtes.
# Descargamos los datasat con la información geografica de las comunas de CABA, la red de subtes y sus estaciones.

# Primero se crea una variable con la información sobre el CRS proyectado para la Ciudad Autónoma de Buenos Aires.
...
#Ahora cargaremos el dataser de comunas y lo tranformamos acorde a las coordenadas geográficas de CABA. 

comuna.ok <- read_sf("Data/comunas_wgs84.shp")

comuna.ok <- st_transform(comuna.ok, crs=caba_proj)

#Haremos lo mismo con las estaciones y líneas de subte. 
...
#Ahora, ya con todas las bases de datos cargadas procederemos a trabajarlas en conjunto para poder cruzar información y avanzar con el análisis.

#Procedemos a incorporar los datos del Cronista en el dataset de Properati para estudiar el valor de los precios en su relacion dólares estadounidenses/pesos argentinos.
...
#A continuación agregaremos una nueva columna que nos indique el precio en pesos que se vendió el departamento al día de la fecha.

departamentos.en.venta.2020.pesos <- departamentos.en.venta.2020.unido %>% 
  mutate (departamentos.en.venta.2020.unido, precioArg= price*valor)%>% #Creamos una nueva columna cuya información sale del cálculo de multriplicar el precio en dólar por el valor del dólar blue
  mutate (departamentos.en.venta.2020.unido, precioXm2= price/surface_total) #Creamos una columna que nos indique el valor del precio por m2.
...
#Al tener los datos unidos en tablas y limpios, empezaremos a graficar la información. 
...
departamentos.en.venta.2020.p.promedio <- departamentos.en.venta.2020.pesos %>% 
  mutate(price = as.numeric(price))  #Configuramos que tome a la variable de la columna Price como numero y no como caracter
...
departamentos.en.venta.2020.p.promedio2 <- departamentos.en.venta.2020.p.promedio %>% 
  group_by(partido) %>% 
  summarise(preciopromm2=mean(precioXm2)) #En este caso realizamos el promedio del precio por m2.

#Procedemos a realizar un gráfico que nos muestre de manera creciente el precio promedio por comuna 
...
ggplot(data=departamentos.en.venta.2020.p.promedio1)+
  ...
  labs(title = "PRECIO PROMEDIO POR COMUNA", subtitle = "CABA", caption = "Fuente: PROPERATI", fill= "PRECIO PROMEDIO",x="COMUNA",y="PRECIO PROMEDIO") + #Agregamos las etiquetas al gráfico
  theme_classic()
...
#Ahora, con la misma lógica, realizamos el gráfico para ver como es el precio promedio del m2.
...
ggplot(data=departamentos.en.venta.2020.p.promedio2)+
  ...
  coord_flip() + 
  labs(title = "PRECIO PROMEDIO DEL M2 VENDIDAS POR COMUNA", subtitle = "CABA", caption = "Fuente: PROPERATI", fill= "PRECIO PROMEDIO M2",x="COMUNA",y="PRECIO PROMEDIO M2") +
  theme_classic()
...
#Ahora analizaremos la cantidad de propiedades vendidas por comuna. 
...
#Es posible observar que la comuna 14 es en la que se vendieron más propiedades.
...
ggplot(departamentos.en.venta.2020.tipo.prop) +
  ...
  coord_flip() +
  labs(title = "TIPO DE PROPIEDADES VENDIDAS", subtitle = "CABA", caption = "Fuente: PROPERATI", fill= "CANTIDAD",x="TIPO DE PROPIEDAD",y="CANTIDAD") + 
  theme_classic()
...
ggplot(departamentos.en.venta.2020.tipo.propXcomuna) + 
  geom_bar(aes(x=partido, weight=cantidad, fill = property_type)) + 
...
#Es posible observar que la comuna 14 fue en la que más se vendieron departamentos, la 12 casas y la 15 PHS. 
...
departamentos.en.venta.2020.mono1 <- departamentos.en.venta.2020.mono %>% 
  ...
  summarise(cantidadmonoamb=n()) #Y contabilizamos el numero de observaciones
...
ggplot()+
  ...
  coord_flip() + 
  labs(title = "MONOAMBIENTES POR COMUNA", subtitle = "CABA", caption = "Fuente: PROPERATI", fill= "CANTIDAD",x="COMUNA",y="CANTIDAD") +
  theme_classic()

#Es posible observar que en la comuna 14 se vendieron más monoambientes, mientras que la 7 fue en la que menos se vendieron. 
...
ggplot()+
  ...
  coord_flip() + 
  labs(title = "PROPIEDADES CON + DE 6 AMB POR COMUNA", subtitle = "CABA", caption = "Fuente: PROPERATI", fill= "CANTIDAD",x="COMUNA",y="CANTIDAD") +
  theme_classic()
...
ggplot(comuna.ok)+
  geom_sf(fill="gray75", color="white") + geom_sf_label(aes(label = COMUNAS), size=2)+ 
...
#Arpovechando que nuestro dataset con los datos de propiedades tiene latitud y longitud para cada observación, lo transformamos en un dataset espacial
...
# Realizamos un par de pasos para que nos quede la columna con información de comunas escritas de igual modo para unirlas
...
# Una vez unidos los dataset con la geometria,  procederemos a mapear la cantidad de departamentos vendidos por comuna y luego el precio promedio de los departamentos vendidos en el 2020 por comuna
...
ggplot()+
  geom_sf(data=geo.cantidad.comuna, aes(fill=cantidad))+  geom_sf_label(data=comuna.ok,aes(label = COMUNAS), size=2)+
  labs(title = "CANTIDAD DE PROPIEDADES VENDIDAS POR COMUNA", subtitle = "CABA 2020", caption = "Fuente:Properati", x = "LATITUD", y = "LONGITUD")

# De esta manera podemos observar que la comuna con más propiedades vendidas en el 2020 fue la comuna 14.

# Ahora realizamos los mismos pasos para ver el precio promedio.
...
ggplot()+
  geom_sf(data=geo.precio.comuna, aes(fill=promm2))+ geom_sf_label(data=comuna.ok,aes(label = COMUNAS), size=2)+
  labs(title = "PROMEDIO DEL M2 POR COMUNA", subtitle = "CABA 2020", caption = "Fuente:Properati", x = "LATITUD", y = "LONGITUD")

# De esta manera podemos ver que las comunas del noreste de la ciudad (1, 2, 14 y 13) son las que vendieron a precios más altos.
#Asimismo, es posible observar cómo los valores disminuyen hacia el sur. 
# Ahora queremos estudiar la relación entre las estaciones de subte y los valores de las propiedades.
...
ggplot()+
  ...
  labs(title = "Red de subtes",
       ...
       caption= "Fuente: Buenos Aires Data",x = "LATITUD", y = "LONGITUD") +
  theme_light()

# Dado que la Comuna 14 fue la que más propiedades vendidas tuvo, con un mercado muy amplio (como vimos en el análisis fue la que más monoambientes y propiedades con más de 6 ambientes se vendieron) y también una de las comunas con valores más elevados, es por eso que haremos foco en ella para estudiar el efecto del subte. 
...
# Transformamos el data set en uno espacial

geoprop <- departamentos.en.venta.2020.pesos %>% 
  st_as_sf(coords = c("lon", "lat"), crs = 4326)

# Vemos como se distribuyeron las propiedadas vendidas en comuna
...
ggplot()+
  ...
  labs(title = "Propiedades en Venta CABA",
       subtitle = "Comunas 14",
       color = "Precio USD",
       caption= "Fuente: Properatti", x = "LATITUD", y = "LONGITUD") +
...
ggplot()+
  ...
  labs(title = "Propiedades en Venta Comuna 14",
       subtitle = "Comuna 14",
       color = "Precio USD",
       caption= "Fuente: Properati", x = "LATITUD", y = "LONGITUD") +
  scale_color_distiller(palette = "YlOrRd", direction = 1) +
  theme_light()

# Ahora le superponemos la cobertura de subtes.
# Para esto deberemos filtrar primero y quedarnos con las estaciones de subte que pasan por la comuna 14 y volver a calcular la cobertura
...
# Vemos el radio de coberttura y cómo varía precio de las propiedades  
...
# Vemos como varía el precio promedio del m2 de las propiedades para evaluar posibles diferencias
...
ggplot()+
  ...
  labs(title = "Valor promedio de la m2 en la Comuna 14",
       ...
       caption= "Fuente: Properati", x = "LATITUD", y = "LONGITUD") +
  theme_light()
# Vemos que sucede lo mismo, el valor de las propiedades aumenta hacia el noreste de la comuna, entendemos que el análisis se ve influenciado por la avenida libertador y su cercanía al río.

#Ahora queremos ver como se distribuye el el tipo de propiedad en relación a la cobertura de subte
...
#Vemos que si bien en la comuna predominan los departamentos, la mayoría de las casas no se encuentran dentro del área de influencia del subte
# Esto se podría atribuir al caracter comercial de la zona.

# Ahora queremos ver cómo es el precio de los monoambientes, entendiendo que este es un mercado particular
...
#Si bien la distribución es bastante pareja encontramos algunos de valor elevado cerca a las estaciones de subte. 

#A modo de concluisón final, fue posible observar cómo los precios se distribuyen en la ciudad. Si bien en las comunas del norte se vendieron más propiedades, cuentan con la particularidad de tener los valores más elevados. 
#Asimismo, si bien notamos que las comunas más costosas cuentan con mayor cobertura de subtes, al centrar nuestro análisis en la comuna 14, notamos que no fue un condicionante.
#Como mencionamos previamente, el precio del m2 puede estar influenciado por la cercanía a grandes espacios verdes, al río y al valor urbano que estos le imprimen a la zona. 
